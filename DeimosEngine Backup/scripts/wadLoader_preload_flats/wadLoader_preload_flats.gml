/// @description wadLoader_preload_flats();
...
for (j = 0; j<ds_list_size(sects); j++ ){
    ...
    if is_undefined(ds_map_find_value(pload_flats,ftex)){
        ...
        var spr=ds_map_find_value(wadFlatTextures,ftex);
        //show_debug_message("spr value: '"+string(spr)+"'");
        if !is_undefined(spr){
            var flat1=wadLoader_buildFlat(spr);   
    ...
    if is_undefined(ds_map_find_value(pload_flats,ctex)){
        ...
        var spr=ds_map_find_value(wadFlatTextures,ctex);
        show_debug_message("spr value: "+string(spr));
        if !is_undefined(spr){
            var flat1=wadLoader_buildFlat(spr);   
