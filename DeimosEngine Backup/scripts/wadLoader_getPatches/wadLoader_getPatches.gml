...
poff=wadLoader_getLumpOfs("PNAMES");
...
for (p=0; p<pcount; p+=1){
poff=wadLoader_getLumpOfs(ds_list_find_value_fixed(wadPNames,p));
...
ds_map_add(patch,"width",buffer_read(wadbuff,buffer_u16));
ds_map_add(patch,"height",buffer_read(wadbuff,buffer_u16));
ds_map_add(patch,"o_left",buffer_read(wadbuff,buffer_u16));
ds_map_add(patch,"o_top",buffer_read(wadbuff,buffer_u16));
...
//data=wadLoader_buildPatch(data,patch_width,patch_height);
